...
In order to overcome this issue, we must avoid mixing EPL and non-EPL devices on the same shared segment. Thus the network is split into more segments by a [router](https://en.wikipedia.org/wiki/Router_\(computing\)) or a [bridge](https://en.wikipedia.org/wiki/Bridging_\(networking\)).
